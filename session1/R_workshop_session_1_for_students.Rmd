...
Based on the manual page, make a matrix called 'mymatrix' with the following parameters and then view it:
...
```{r}
 # subset 'fhs', including only complete rows
#summary(fhs.clean)
#str(fhs.clean)
```
...
```{r}
#hist() # histogram of the diastolic blood pressure
#hist() # histogram of the systolic blood pressure
```
